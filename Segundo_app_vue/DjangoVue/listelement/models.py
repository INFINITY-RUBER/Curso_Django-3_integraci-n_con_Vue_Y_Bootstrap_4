...
from django.utils.text import slugify # para que complete la url limpia
import os
...
class Element(models.Model):  
    ...
    price = models.DecimalField(max_digits=10,decimal_places=2, default=6.10)# 12345678 
    ...
    def get_discount(self, coupon):
        return (coupon.discount / Decimal(100) * self.price) # ej: 10 / 1000 = 0.1 * 10$ = 1
...
class ElementImages(models.Model):
    ...
    base_cover_name  = models.CharField(max_length=155, default='')
...
@receiver(models.signals.post_delete, sender=ElementImages) # decorador que escucha el evento al borrar
...
@receiver(models.signals.pre_save, sender=ElementImages) # decorador que escucha el evento al borrar
    ...
    if not old_file == new_file:        
        if instance.cover:
            if os.path.isfile(old_file.path):
                ...
                # print("se borro: ", old_file.path)

        (root, _) = os.path.splitext(old_file.path)        
        extra_file = root+".custom.jpeg"
        # print(extra_file)
